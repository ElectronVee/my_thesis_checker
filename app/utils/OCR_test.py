...
def OCR_lmj(img_path):
    ...
    # 识别图片中的数字和字母
    text = pytesseract.image_to_string(out, lang='chi_sim+eng')
...
def main():
    # 识别指定文件目录下的图片
    # 图片存放目录figures
    # dir = 'D://pythonProject/test/test_png'
    #
    ...
    expected_clean = expected_names.strip().replace(" ", "").upper()

    print(f"识别结果: {text_clean}")
    print(f"预期结果: {expected_clean}")

    if text_clean == expected_clean:
        print("识别成功，文字完全匹配！")
    else:
        print("识别失败，文字不匹配！")
